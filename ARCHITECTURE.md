...
The core can reuse Blizzard's player cast bar for instant-spell global cooldowns. A player-only spellcast event identifies instant casts, the following cooldown update supplies the start-recovery duration, and Blizzard's normal cast-bar update and fade paths animate the result. Real casts and channels take priority. The same setting hooks ConsolePort action-button cooldown updates and clears only cooldowns identified by the game as global cooldowns.
