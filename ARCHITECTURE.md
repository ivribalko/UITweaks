...
The ConsolePort tabs module listens for L1 and R1 only while a supported window is visible and ConsolePort's interface cursor is active. It cycles through the visible tabs in the Adventure Guide, Auction House, Bank, Character, Dungeons & Raids, Map, Talents and Spellbook, and Warband Collections windows, skipping tabs unavailable to the current character or game mode. It overlays ConsolePort's active-controller L1 and R1 icons at the ends of each tab group and refreshes them when tab availability or the active controller icon set changes. All unrelated controller input continues through the normal input path.
