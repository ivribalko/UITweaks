...
- `UITweaksAggroRadius.lua` estimates hostile mouseover mob aggro distance and appends it to the unit tooltip while out of combat.
...
The core applies separate in-combat and out-of-combat opacity settings to the player frame, player cast bar, target frame, and Blizzard's Extra Abilities container. A cast-bar alpha hook preserves the active setting when Blizzard updates its own cast fade animations.

The aggro radius module hooks Blizzard's unit-tooltip data pipeline. For living hostile NPC mouseovers, it estimates the radius from the player-to-mob level difference and clamps the result to the game's normal minimum and maximum bounds. Skull-level bosses, which report level -1, use the game's conventional player-level-plus-three value. Item range checks estimate the current-distance interval because WoW does not expose exact hostile-unit distance. The tooltip subtracts the aggro radius from that interval to show the remaining distance before aggro, only while the player is out of combat.

The Cooldown Manager overlay module matches Blizzard cooldown entries to ConsolePort action buttons by spell ID. It keeps the native Blizzard item frames and their update behavior, but anchors them to the matching ConsolePort buttons through separate visibility containers so the original Cooldown Manager visibility settings still apply. Known tracked auras whose spell IDs differ from their actions, including Demonic Core, Infernal Bolt, and Ruination, are linked to their base, intermediate, and replacement action spell IDs, following active override chains recursively. BuffIcon items are processed before cooldown items so active ticking buffs take priority when both can match the same button. Matched buttons keep their secure input frame, persistent ConsolePort frame texture, native spell-activation glows, and gamepad hotkeys while their original action artwork is hidden, and each native item samples the button's effective opacity during existing overlay updates. An optional setting hides native countdown numbers and applies the Essential Cooldown yellow swipe color while matched items display active buffs, then restores the numbers when those items display cooldowns. Active-buff appearance is reapplied synchronously after native item refreshes, while structural rematching remains deferred. When the overlay setting is enabled, the module also adds a Clear button to the left of Blizzard's Revert button in Advanced Cooldown Settings; it moves every entry in the tracked buff, tracked bar, essential cooldown, and utility cooldown categories to the matching hidden category through Blizzard's layout data provider.
...
The ConsolePort tabs module listens for L1 and R1 only while a supported window is visible and ConsolePort's interface cursor is active. It cycles through the visible tabs in the Adventure Guide, Auction House, Bank, Character, Dungeons & Raids, Map, Talents and Spellbook, Transmogrify, and Warband Collections windows, skipping tabs unavailable to the current character or game mode. It overlays ConsolePort's active-controller L1 and R1 icons at the ends of each tab group and refreshes them when tab availability or the active controller icon set changes. All unrelated controller input continues through the normal input path.
